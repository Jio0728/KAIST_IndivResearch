...
import StyleCLIP.criteria.clip_loss as clip_loss
from StyleCLIP.criteria import id_loss
from StyleCLIP.mapper.datasets.latents_dataset import LatentsDataset, StyleSpaceLatentsDataset
from StyleCLIP.mapper.styleclip_mapper import StyleCLIPMapper
from StyleCLIP.mapper.training.ranger import Ranger
from StyleCLIP.mapper.training import train_utils
from StyleCLIP.mapper.training.train_utils import convert_s_tensor_to_list

from StyleNeRF.training.networks import MappingNetwork
...
class Coach:
	def __init__(self, opts):
		...
		self.train_dataloader = DataLoader(self.train_dataset, #여기서 latents 데이터셋 생성
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				if self.opts.work_in_stylespace:
					...
					w = batch #생성한 데이터셋에서 그대로 받아옴.
					w = w.to(self.device)
				if len(w) >1: #batch size ==2
					inject_index = random.randint(1, 17)
					latent1 = w[0].unsqueeze(1).repeat(1, inject_index, 1)
					latent2 = w[1].unsqueeze(1).repeat(1, 18-inject_index, 1)
					w = torch.cat([latent1, latent2], 1) 
				...
				with torch.no_grad():
					...
					camera_matrices = self.net.decoder.get_camera_traj(0, 1, device=self.device)   
					x = self.net.decoder(styles=w, camera_matrices=camera_matrices)	
					# print(x.shape)
				if self.opts.work_in_stylespace:          
					delta = self.net.mapper(w)
					w_hat = [c + 0.1 * delta_c for (c, delta_c) in zip(w, delta)]
					x_hat, _, w_hat = self.net.decoder(styles=w_hat, input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, input_is_stylespace=True)
				else:
					...
					x_hat = self.net.decoder(styles=w_hat, camera_matrices=camera_matrices)          
				# print(w.shape, x.shape, w_hat.shape, x_hat.shape)
	...
	def configure_datasets(self):
		if self.opts.NeRFMapping:
			print("NeRF")
			train_latents_z = np.random.RandomState(self.opts.train_seed).randn(self.opts.train_dataset_size, 512)
			train_latents = []
			for b in range(self.opts.train_dataset_size // self.opts.batch_size):
				with torch.no_grad():
					train_latents_b = self.net.olddecoder.mapping(torch.from_numpy(train_latents_z[b * self.opts.batch_size: (b + 1) * self.opts.batch_size]).to(self.device), None, truncation_psi=0.7)  
		...
		if self.opts.NeRFMapping:
			print("NeRF")			
			test_latents_z = np.random.RandomState(self.opts.test_seed).randn(self.opts.test_dataset_size, 512)
			test_latents = []
			for b in range(self.opts.test_dataset_size // self.opts.batch_size):
				with torch.no_grad():
					test_latents_b = self.net.olddecoder.mapping(torch.from_numpy(test_latents_z[b * self.opts.test_batch_size: (b + 1) * self.opts.test_batch_size]).to(self.device), None, truncation_psi=0.7)  
